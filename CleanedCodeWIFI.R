...
# Setting the data
Train_PCs_20$LONGITUDE <- WifiTrain$LONGITUDE
TEST_PCs_206$LONGITUDE <- WifiTestNoNA529col$LONGITUDE

# Splitting the data into training and testing set
set.seed(123)
Train_PCs_206_Sample <- Train_PCs_206[sample(1:nrow(Train_PCs_206), 100, replace = FALSE),]
...
write.csv(rfPredictionLONGI, file = "rfPredictionLONGI.csv", row.names = TRUE)
...
# Exporting the prediction
write.csv(rfPredictionFLOOR, file = "rfPredictionFLOOR.csv", row.names = TRUE)
...
set.seed(123)
WifiTrainNoNAModeling_Sample <- WifiTrainNoNAModeling[sample(1:nrow(WifiTrainNoNAModeling), 100, replace = FALSE),]
...
rfGrid <- expand.grid(mtry=c(63))
rfFitModelBUILD <- train(BUILDINGID~ .,
                         data = training,
                         method = "rf",
                         trControl=fitControl,
                         tuneGrid=rfGrid
...
# Exporting the prediction 
write.csv(rfPredictionBUILD, file = "rfPredictionBUILD.csv", row.names = TRUE)
...
# Exporting the prediction
write.csv(rfPredictionLat, file = "rfPredictionLat.csv", row.names = TRUE)
